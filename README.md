...
It is important that in either case, the halting instructions are only executed once it can be ensured that collators have had enough time to obtain the chain data of the relaychain that handles the shell parachain. In addition, in both cases there is potential another constraint that has to be respected. Should the live parachain use the author-inherent pallet from the Nimbus framework, a storage element (`HighestSlotSeen`) has to be reset if the relaychain of the shell parachain has a lower block number than the relaychain of the live parachain, otherwise the chain won't produce blocks after the migration.
